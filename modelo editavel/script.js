...
function estimateBPM(buffer) {
  ...
  // Analisa apenas os primeiros 30s
  const maxSamples = Math.min(buffer.length, sampleRate * 30);
  const data = buffer.getChannelData(0);

  // Janela de ~10ms para detectar batidas
  ...
  // Suaviza com media movel
  const smooth = [];
  const w = 4;
  for (let i = w; i < energies.length - w; i++) {
    let s = 0;
    for (let j = -w; j <= w; j++) s += energies[i + j];
    smooth.push(s / (2 * w + 1));
  }
  ...
  // Gap minimo entre picos = 250ms (nao mais de 240 BPM)
  const minGap = Math.floor(0.25 / 0.01);
  const peaks = [];
  let lastPeak = -minGap;
  ...
  if (peaks.length < 4) return 160;

  const intervals = [];
  for (let i = 1; i < Math.min(peaks.length, 80); i++) {
  ...
  // Cada unidade = 10ms -> BPM = 60 / (median * 0.01)
  let bpm = Math.round(60 / (median * 0.01));

  // Normaliza para faixa musical realista (120-200 BPM)
  while (bpm > 200) bpm = Math.round(bpm / 2);
  while (bpm < 120) bpm = bpm * 2;
...
function updateStats(speedPct) {
  ...
  // pace mínimo = passada máxima → você corre mais rápido
  // pace máximo = passada mínima → você corre mais devagar
  el("stat-pace-min").textContent = metrics.paceFast;
  el("stat-pace").textContent     = metrics.paceCenter;
  el("stat-pace-max").textContent = metrics.paceSlow;

  el("stat-sub").textContent =
    `cadência: ${metrics.cadencia} passos/min · ${metrics.syncLabel}`;

  const passadaBox = el("stat-passada-box");
  const passadaTxt = el("stat-passada-txt");
  if (audioBuffer) {
    passadaBox.style.display = "block";
    passadaTxt.textContent = `tamanho da passada: ${perfil.passada.toFixed(2)} m`;
  } else {
    passadaBox.style.display = "none";
  }
